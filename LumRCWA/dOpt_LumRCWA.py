...
params_2d = {
          ...
          'wavelength-center' : 480e-9, # MUST CHANGE INDICES AS WELL (coded after opt. loop) 
          'wavelength-FWHM' : 20e-9, 
          'wavelength-points' : 5, # Note: I believe points end up getting evenly spaced in frequency, not wavelength 
          'QW-xy-mesh' : 50, 
          'QW-z-mesh': 50, 
          'k-mesh': 26, 
          ...
          'layer-thicknesses' : [1e-6, None, None, 0.120e-6, 1e-6], # GaN thicknesses can be variable param 
          'layer-materials' : ["Al2O3 - Palik", "GaN - custom", "GaN - custom", 'ITO - custom', 'etch'],
          #'QW-count' : 3, 
          'QW-relative-intensities' : [0.45, 0.33, 0.22], # relative intensities of QWs 
...
# =============================================================================
          ...
          'QW-layer' : 2, # Which layer is the QW in: 0, 1, 2, ... The QW will be optimally placed within 50-300 nm of the bottom of this layer 
          'ribbon-count' : 1, # number of nanoribbons to etch 
          'notch-count' : 1, 
          'target-k' : (0, 0) # (kx, ky) 
...
min_mesa_width = 0.050e-6  
min_period = params_2d['wavelength-center'] * 0.50 
max_period = params_2d['wavelength-center'] * 1.50 
...
#                    'type' : 'range', 
#                    'bounds' : [0.050e-6, max_period/params_2d['ribbon-count'] - min_mesa_width] 
#                    #'bounds' : [0.0, params_2d['period'][0]/params_2d['ribbon-count'] - min_mesa_width] 
#                        })
# =============================================================================
# Allow for optimizable GaN thickness 
var_2d.append({'name':'uniform-GaN-thickness', 'type':'range', 'bounds':[2.0e-6, 4.0e-6]})
var_2d.append({'name':'etched-GaN-thickness', 'type':'range', 'bounds':[0.500e-6, 1.100e-6]})
        ...
        for n in [v['name'] for v in variables]:
            if n[-9:] == 'thickness': 
                if n[:3] == 'uni': 
                    ...
                    params['layer-thicknesses'][2] = np.round(parameterization.get(n), 3) 
                else:
...
# =============================================================================
        ...
        q = mp.Queue() 
        p = mp.Process(target = directivityLumerical.FoM, args = (params, q,))
...
# =============================================================================
# # Testing that new FoM works (broadband loop within FoM_2d instead of D_z_2d or I_z_2d) 
# params_2d['reciprocity-N'] = 14
# params_2d['Fourier-N'] = 14
# params_2d['layer-thicknesses'] = [0, 0.996 * 48/54 * 2, 0.140 * 48/54, 0.12, 0]
# params_2d['period'] = [0.424 * 48/54, 0.673 * 48/54]
# params_2d['ribbon-centers'] = [0.3460707981203343] 
# params_2d['ribbon-widths'] = [0.3693413459534806]
# params_2d['notch-centers'] = [0.3983340637735282] 
...
# # weights = np.stack((wavelength_range, np.exp(-(wavelength_range - params_2d['wavelength'])**2 / (spectral_FWHM**2 / (4*np.log(2))) )), axis=1)
# # =============================================================================
# run = False 
# if run: 
...
# 
# # Plot emission profile across spectral FWHM
# import matplotlib.pyplot as plt  
# kx_range = [-1.3, 1.3] 
# ky_range = [-1.3, 1.3] 
# plt.imshow(I_total.T, extent = [kx_range[0], kx_range[-1], ky_range[0], ky_range[-1]], cmap = 'inferno')
# plt.xlabel('$k_x$/$k_0$')
# plt.ylabel('$k_y$/$k_0$')
# plt.title('dual-pol emissions from QW depth ' + str(QW_depth) + ' um \n Emission across spectral FWHM \n Etch damage = 10 nm')
# plt.colorbar() 
# plt.show() 
# =============================================================================
